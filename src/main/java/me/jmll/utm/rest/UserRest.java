...
public class UserRest {
	...
	public Map<String, Object> getUsersJSON() {
		...
		for (User u : userService.getUsers()) {
			...
			links2.add(new Link(builder.path("").build().toString() + "/" + user, user));
		}
		
		
		Map<String, Object> response = new HashMap<>();
		response.put("_links", links);
		response.put("data", links2);
		// }
		return response;
	}
	...
	public UserLinkListResource getUsersXML() {
		// Escribe tu código aquí {
		ServletUriComponentsBuilder builder = ServletUriComponentsBuilder.fromCurrentServletMapping();
		UserLinkListResource uls = new UserLinkListResource();
		
		uls.addLink(new Link(builder.path("/").build().toString(), "api"));
		uls.addLink(new Link(builder.path("/user").build().toString(), "user"));

		for (User u : userService.getUsers()) {
			String user = u.getUsername();
			uls.addUserLink(new Link(builder.path("").build().toString() + "/" + user, user));
		}
	...
	public Map<String, Object> getUserJSON(@PathVariable("username") String username) {
		...
		List<Link> links = new ArrayList<Link>();
		links.add(new Link(builder.path("/user").build().toString(), "user"));
		links.add(new Link(builder.path("/" + user.getUsername()).build().toString(), "self"));
		
		Map<String, String> dataA = new HashMap<>();
		dataA.put("fullName", user.getFullName());
		dataA.put("password", user.getPassword());
		dataA.put("username", user.getUsername());
		
		
		Map<String, Object> response = new HashMap<>();
		response.put("_links", links);
		response.put("data", dataA);
		// }
		return response;
	}
	...
	public UserResource getUserXML(@PathVariable("username") String username) {
		...
		resource.addLink(new Link(builder.path("/" + user.getUsername()).build().toString(), "self"));
